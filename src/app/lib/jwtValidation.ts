...
export async function validateJWT(
  ...
  try {
    ...
    // Handle JWKS fetch backoff errors (common during development)
    if (error.name === "WaitPeriodNotYetEndedJwkError") {
      console.warn("JWT verifier is in cooldown, skipping validation");
      // Return as valid temporarily to avoid blocking the user
      // The token will be re-validated on next page load
      return {
        isValid: true,
